...
def rotate_counterclockwise(shape):
    ...
    print("initial: " + str(shape))

    new_shape_array = [[shape[y][x] for y in range(len(shape))] for x in range(len(shape[0]) - 1, -1, -1)]

    for row in range(len(shape)):
        print("Row[] -> ")
        print(row)
        print("Cell is ->")
        for cel in range(len(shape[0]) - 1, -1, -1):
            print(shape[row][cel])
